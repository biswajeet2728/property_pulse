...
const PropertyContactForm = ({ property }) => {
  ...
  // Define the form action handler
  const formAction = async (formData) => {
    try {
      await addMessage(formData);
      setState({ submitted: true, error: null });
    } catch (err) {
      setState({ submitted: false, error: err.message || 'Something went wrong' });
    }
  };
  ...
  return (
    <div className="bg-white p-4 mt-4 rounded shadow">
      ...
      >
        ...
        <div className="mb-4">
          <label htmlFor="body" className="form-label fw-bold">Message</label>
          <textarea
            id="body"
            name="body"
            rows="6"
